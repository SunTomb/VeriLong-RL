    ...
    <div className="app-shell">
      <header className="top-nav">
        ...
        <nav aria-label="Main navigation">
          <button className={page === 'home' ? 'nav-active' : ''} type="button" onClick={() => setPage('home')}>Home</button>
          <button className={page === 'demo' ? 'nav-active' : ''} type="button" onClick={() => setPage('demo')}>Demo</button>
        </nav>
      </header>

      {error ? <main className="section"><p className="error" role="alert">{error}</p></main> : null}
      {!error && !summary ? <main className="section"><p>Loading VeriLong-RL…</p></main> : null}
